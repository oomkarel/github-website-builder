...
function HeroBlock({
  ...
  return <section className="relative min-h-[70vh] flex items-center justify-center text-center overflow-hidden" style={{
    backgroundImage: backgroundImage ? `url(${backgroundImage})` : undefined,
    backgroundSize: 'cover',
    backgroundPosition: 'center'
  }}>
      {/* Gradient overlay for better text readability */}
      {data.background_image && <div className="absolute inset-0 bg-gradient-to-b from-black/70 via-black/50 to-black/70" />}
      <div className="relative z-10 max-w-4xl mx-auto px-6 py-24">
        {data.title && <h1 className={cn("text-4xl md:text-5xl lg:text-6xl font-bold mb-6 leading-tight animate-fade-up", data.background_image ? "text-white drop-shadow-lg" : "text-foreground")}>
            {data.title}
          </h1>}
        {data.subtitle && <p className={cn("text-xl md:text-2xl mb-10 max-w-2xl mx-auto leading-relaxed animate-fade-up", data.background_image ? "text-white/90" : "text-muted-foreground")} style={{
        animationDelay: '0.1s'
      }}>
            {data.subtitle}
          </p>}
        ...
        animationDelay: '0.2s'
            ...
            {data.primary_button_text && <Button size="lg" asChild className="text-lg px-8 py-6 h-auto shadow-lg hover:shadow-xl transition-shadow">
                <Link to={data.primary_button_link || '#'}>{data.primary_button_text}</Link>
              </Button>}
            {data.secondary_button_text && <Button size="lg" variant="outline" asChild className={cn("text-lg px-8 py-6 h-auto", data.background_image && "bg-white/10 border-white/50 text-white hover:bg-white/20 backdrop-blur-sm")}>
...
function TextBlock({
  ...
  return <section className="py-6 md:py-8 px-6">
      <div className="max-w-4xl mx-auto">
        {/* Render title if present */}
        {data.title && <h2 className="text-2xl md:text-3xl font-bold text-foreground mb-6">
            {data.title}
          </h2>}
        <div className="prose prose-lg dark:prose-invert max-w-none text-left prose-h1:md:text-3xl prose-h1:text-foreground prose-h1:mb-4 prose-h1:mt-0 /* In CMS content, body text is sometimes authored as H2/H3. Style them like description/subtitle (not big headings). */ prose-h2:md:text-lg prose-h2:text-muted-foreground prose-h2:leading-relaxed prose-h2:mb-4 prose-h2:mt-0 prose-h3:md:text-lg prose-h3:text-muted-foreground prose-h3:leading-relaxed prose-h3:mb-4 prose-h3:mt-0 prose-p:text-muted-foreground prose-p:md:text-lg prose-p:leading-relaxed prose-p:mb-4 prose-strong:text-foreground prose-a:text-primary prose-a:no-underline hover:prose-a:underline prose-ul:my-4 prose-li:text-muted-foreground prose-li:leading-relaxed prose-blockquote:border-l-primary prose-blockquote:bg-muted/30 prose-blockquote:py-2 prose-blockquote:rounded-r-lg text-lg font-normal" dangerouslySetInnerHTML={{
...
function TableBlock({
  ...
  return <section className="py-6 md:py-10 px-6">
      <div className="max-w-4xl mx-auto">
        {/* Table Title */}
        {data.title && <h3 className="text-2xl md:text-3xl font-bold text-foreground mb-3 text-center">
            {data.title}
          </h3>}
        
        {/* Table Description */}
        {data.description && <p className="text-muted-foreground text-center mb-6 max-w-2xl mx-auto leading-relaxed">
            {data.description}
          </p>}
      ...
      <div className="relative z-10 container mx-auto px-6">
        <div className="max-w-3xl mx-auto text-center text-primary-foreground">
          ...
          {(primaryButtonText || secondaryButtonText) && <div className="flex flex-wrap gap-4 justify-center">
              {primaryButtonText && <Button size="lg" variant="secondary" asChild className="text-lg px-8 py-6 h-auto shadow-lg hover:shadow-xl transition-all">
                  <Link to={primaryButtonLink}>
                    {primaryButtonText}
                    <ArrowRight className="ml-2 h-5 w-5" />
                  </Link>
                </Button>}
              {secondaryButtonText && <Button size="lg" variant="outline" asChild className="text-lg px-8 py-6 h-auto bg-white/10 border-white/50 text-white hover:bg-white/20 backdrop-blur-sm">
...
function FeaturesBlock({
  ...
  return <section className="py-16 md:py-20 px-6">
      <div className="max-w-6xl mx-auto">
        {(data.title || data.subtitle) && <div className="text-center mb-12">
            {data.title && <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">{data.title}</h2>}
            {data.subtitle && <p className="text-lg text-muted-foreground max-w-2xl mx-auto">{data.subtitle}</p>}
          </div>}
        ...
        <div className="flex flex-wrap justify-center gap-8">
          ...
          return <Card key={idx} className="text-center group hover:shadow-lg transition-all duration-300 hover:-translate-y-1 border-border/50 w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.333%-1.5rem)] max-w-sm">
                <CardContent className="pt-8 pb-6">
                  {IconComponent && <div className="w-14 h-14 mx-auto mb-5 rounded-xl bg-primary/10 flex items-center justify-center group-hover:bg-primary/20 transition-colors">
                      <IconComponent className="h-7 w-7 text-primary" />
                    </div>}
                  {item.title && <h3 className="text-xl font-semibold mb-3 text-foreground">{item.title}</h3>}
  ...
  return <section className="py-16 px-6">
      <div className="max-w-3xl mx-auto">
        <Accordion type="single" collapsible className="space-y-4">
          {items.map((item: any, idx: number) => <AccordionItem key={idx} value={`item-${idx}`} className="border rounded-lg px-4">
  ...
  return <section className="py-16 px-6">
      <div className="max-w-6xl mx-auto">
        <div className={cn("grid gap-8", plans.length === 1 && "grid-cols-1 max-w-md mx-auto", plans.length === 2 && "grid-cols-1 md:grid-cols-2", plans.length >= 3 && "grid-cols-1 md:grid-cols-2 lg:grid-cols-3")}>
          {plans.map((plan: any, idx: number) => <Card key={idx} className={cn("relative flex flex-col", plan.is_popular && "border-primary shadow-lg scale-105")}>
              ...
              <CardContent className="flex-1 flex flex-col">
                <ul className="space-y-3 mb-6 flex-1">
                  {(plan.features || []).map((feature: string, fidx: number) => <li key={fidx} className="flex items-start gap-2">
  ...
  return <section className="py-16 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {members.map((member: any, idx: number) => <Card key={idx} className="text-center overflow-hidden">
              ...
              <CardContent className="pt-4">
                ...
                {member.linkedin && <a href={member.linkedin} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm text-muted-foreground hover:text-primary transition-colors">
...
function StatsCounterBlock({
  ...
  return <section className="py-16 px-6 bg-primary text-primary-foreground">
      <div className="max-w-6xl mx-auto">
        <div className={cn("grid gap-8 text-center", stats.length === 1 && "grid-cols-1", stats.length === 2 && "grid-cols-2", stats.length === 3 && "grid-cols-3", stats.length >= 4 && "grid-cols-2 md:grid-cols-4")}>
          {stats.map((stat: any, idx: number) => <div key={idx} className="space-y-2">
...
function ContactFormBlock({
  ...
  return <section className="py-16 px-6">
      <div className="max-w-xl mx-auto">
        {(data.title || data.description) && <div className="text-center mb-8">
...
export default function BlockRenderer({
  ...
  return <div className="block-renderer">
      {blocks.map(block => <div key={block.id} className={getSectionBg(block.type)}>
          {renderBlock(block)}
        </div>)}
    </div>;
}